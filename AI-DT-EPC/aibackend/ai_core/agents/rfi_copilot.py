...
{RESPONSE_SCHEMA_HINT}"""
        ...
        messages = [{"role": "system", "content": RFI_COPILOT_SYSTEM_PROMPT}]
        for turn in chat_history or []:
            role = turn.get("role")
            content = turn.get("content", "")
            if role == "assistant":
                messages.append({"role": "user", "content": f"[Prior assistant answer]: {content}"})
            else:
                messages.append({"role": "user", "content": f"[Prior user question]: {content}"})
