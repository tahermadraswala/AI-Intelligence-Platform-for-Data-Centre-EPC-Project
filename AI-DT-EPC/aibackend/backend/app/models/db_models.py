...
class SupplyChainRiskRun(Base):
    """A single Supply Chain Visibility & Risk Agent run over procurement/shipment data."""

    __tablename__ = "supply_chain_risk_runs"

    id: Mapped[str] = mapped_column(String, primary_key=True, default=_uuid)
    source: Mapped[str] = mapped_column(String, default="")  # e.g. filename or "mock"
    overall_on_time_percentage: Mapped[float] = mapped_column(Float, default=0.0)
    total_value_at_risk_usd: Mapped[int] = mapped_column(Integer, default=0)
    shipment_risks: Mapped[list] = mapped_column(JSON, default=list)
    assumptions: Mapped[list] = mapped_column(JSON, default=list)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow)


class CommissioningQARun(Base):
    """A single Commissioning QA Copilot run over equipment test/NCR data."""

    __tablename__ = "commissioning_qa_runs"

    id: Mapped[str] = mapped_column(String, primary_key=True, default=_uuid)
    source: Mapped[str] = mapped_column(String, default="")  # e.g. filename or "mock"
    overall_readiness_percentage: Mapped[float] = mapped_column(Float, default=0.0)
    go_no_go_recommendation: Mapped[str] = mapped_column(String, default="")
    equipment_assessments: Mapped[list] = mapped_column(JSON, default=list)
    critical_blockers: Mapped[list] = mapped_column(JSON, default=list)
    assumptions: Mapped[list] = mapped_column(JSON, default=list)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow)
